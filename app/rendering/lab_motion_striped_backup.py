...
def build_lab_background(path: Path, *, width: int, height: int) -> None:
    image = Image.new("RGBA", (width, height), DARK_BG)
    base = ImageDraw.Draw(image)

    # soft vertical texture
    for x in range(0, width, 8):
        alpha = 4 + int(5 * (0.5 + 0.5 * math.sin(x / 35)))
        base.line((x, 0, x, height), fill=(30, 55, 95, alpha), width=2)

    # glow
    glow = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    gdraw = ImageDraw.Draw(glow)
    glow_center = (int(width * 0.64), int(height * 0.42))

    for radius, alpha in [(520, 40), (380, 55), (240, 75), (140, 95)]:
        x, y = glow_center
        gdraw.ellipse(
            (x - radius, y - radius, x + radius, y + radius),
            fill=(48, 116, 220, alpha),
        )
    ...
    main_points = [
        ...
        (int(width * 0.72), int(height * 0.27)),
        (int(width * 0.95), int(height * 0.14)),
    ]
    ghost_1 = [
        (-20, int(height * 0.70)),
        (int(width * 0.31), int(height * 0.55)),
        (int(width * 0.58), int(height * 0.37)),
        (int(width * 0.82), int(height * 0.20)),
    ]
    ghost_2 = [
        (20, int(height * 0.77)),
        (int(width * 0.39), int(height * 0.60)),
        (int(width * 0.69), int(height * 0.40)),
        (int(width * 0.86), int(height * 0.23)),
    ]

    odraw.line(ghost_1, fill=(120, 160, 220, 65), width=3)
    odraw.line(ghost_2, fill=(40, 120, 240, 110), width=3)
    odraw.line(main_points, fill=BLUE, width=5)

    dashed = [
        (40, int(height * 0.79)),
        (int(width * 0.42), int(height * 0.61)),
        (int(width * 0.86), int(height * 0.24)),
    ]
    ...
    for i in range(len(dashed) - 1):
        ...
        for j in range(segments):
            if j % 2 == 0:
                sx1 = x1 + (x2 - x1) * (j / segments)
                sy1 = y1 + (y2 - y1) * (j / segments)
                sx2 = x1 + (x2 - x1) * ((j + 0.55) / segments)
                sy2 = y1 + (y2 - y1) * ((j + 0.55) / segments)
                odraw.line((sx1, sy1, sx2, sy2), fill=(40, 120, 240, 155), width=3)

    for px, py in [main_points[2], main_points[3], main_points[4]]:
        odraw.ellipse((px - 8, py - 8, px + 8, py + 8), fill=SOFT_BLUE)
        odraw.ellipse((px - 22, py - 22, px + 22, py + 22), fill=(60, 130, 235, 28))
